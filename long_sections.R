...
draw.sites.chart <- function(input.data.frame,
    ...
    ### draw the plot window and label the axes
    plot.new()
    par(mai=c(1,1,1,1), cex=4)
    plot.window(xlims, ylims)
    #box()
    title(main = paste(catchment.name, 'Long Section'),
          sub  = source.ref,
          xlab = paste(round(tail(distance.to.bottom, 1)/1000, 2), 'km'),
          ylab = paste(round(y.top-y.bot[1], 2), 'm'), cex=3)
    ### Draw the thalweg
    lines(cbind(distance.to.bottom, thalweg.elevation), lwd=2, col='#5876b0')
    ### Draw the topbank
    #lines(cbind(distance.to.bottom, topbank.elevation), col='#4e4e4e', lwd=2)
    ### Draw vertical lines to represent the new structures
    text(thalweg.elevation~distance.to.bottom, labels=site.code,
         adj=c(-0.2,1.2))
...
molonglo.sites <- read.csv('molonglo_structure_locations.csv')

colnames(molonglo.sites)[1] <- 'site.code'

molonglo.sites <- molonglo.sites[!is.na(molonglo.sites$site.code), ]

draw.sites.chart(molonglo.sites, 'Molonglo River')
